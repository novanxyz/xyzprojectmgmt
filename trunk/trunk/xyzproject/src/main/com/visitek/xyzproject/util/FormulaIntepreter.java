...
public class FormulaIntepreter {
	...
	public FormulaIntepreter(String formula) throws FormulaException {
		...
		// automatically parse formula execution order by surround it by
		// parenthese
	 ...
	 * @throws FormulaException
	 */
	public Object eval(Object input) throws FormulaException, ScriptException,
	...
	private int findEnclosure(int i, String exp, char openchar, char closechar) {
		...
		if (closechar != openchar && o < r)
			r = exp.indexOf(closechar, findEnclosure(o, exp, openchar,
					closechar) + 1);
		return r;
	}
		...
		for (int i = 0; i < formula.length(); i++)
			if (formula.charAt(i) == '(') {
				...
				if (kt < i)
					...
					t = formula.substring(0, i - 1)
							+ parseExpression(formula.substring(i + 1, kt - 1))
							+ formula.substring(kt + 1, formula.length());
