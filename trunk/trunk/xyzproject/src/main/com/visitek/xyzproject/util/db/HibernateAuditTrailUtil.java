...
public class HibernateAuditTrailUtil implements Interceptor {
	...
	public void logChanges(Object newObject, Object existingObject,
		...
		try {
			...
			if (event.equals(UPDATE)) {
				...
				Field[] oldfields = existingObject.getClass()
						.getDeclaredFields();
				...
				for (int i = 0; i < newfields.length; i++) {
					...
					if (newObj.getClass().getSuperclass() != null
							&& !newObj.getClass().getSuperclass().getName()
	...
	public boolean onFlushDirty(Object arg0, Serializable arg1, Object[] arg2,
		...
		if (arg0 instanceof AbstractPersistentClass) {
			...
			Object preUpdateState = session.get(obj.getClass(),
					persistedObjectId);

			try {

				logChanges(arg0, preUpdateState, null, persistedObjectId
						.toString(), UPDATE);
	...
	public void postFlush(Iterator arg0) throws CallbackException {
		...
		try {
			for (Object element : inserts) {
				AuditTrail logRecord = (AuditTrail) element;
				session.save(logRecord);
			}
			for (Object element : updates) {
				AuditTrail logRecord = (AuditTrail) element;
				session.save(logRecord);
			}
			for (Object element : deletes) {
				AuditTrail logRecord = (AuditTrail) element;
				session.save(logRecord);
			}
